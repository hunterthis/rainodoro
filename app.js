...
function createRipple(){ const water = document.getElementById('water'); if(!water) { console.log('Water element not found'); return; } const ripple = document.createElement('div'); ripple.className = 'ripple'; const size = Math.random() * 20 + 10; const left = Math.random() * 100; const top = Math.random() * 100; ripple.style.width = size + 'px'; ripple.style.height = size + 'px'; ripple.style.left = left + '%'; ripple.style.top = top + '%'; ripple.style.animation = 'ripple 0.8s ease-out forwards'; water.appendChild(ripple); console.log('Ripple created'); setTimeout(() => ripple.remove(), 800); }

function createBubble(){ const water = document.getElementById('water'); if(!water) { console.log('Water element not found for bubble'); return; } const bubble = document.createElement('div'); bubble.className = 'bubble'; const size = Math.random() * 4 + 2; const left = Math.random() * 90 + 5; bubble.style.width = size + 'px'; bubble.style.height = size + 'px'; bubble.style.left = left + '%'; bubble.style.bottom = '5%'; bubble.style.animation = `bubble-rise ${Math.random() * 1 + 1}s ease-in forwards`; water.appendChild(bubble); console.log('Bubble created'); setTimeout(() => bubble.remove(), 2000); }
...
function makeRain(){ const overlay = document.getElementById('rainOverlay'); if(!overlay) return; overlay.innerHTML=''; for(let i=0;i<80;i++){ const d=document.createElement('div'); d.className='raindrop'; d.style.left = (Math.random()*100)+'%'; d.style.top = (Math.random()*10)+'%'; d.style.height = (8+Math.random()*18)+'px'; d.style.opacity = 0.3+0.4*Math.random(); d.style.animation = `drop ${0.9+Math.random()*1.6}s linear ${Math.random()*1.5}s infinite`; overlay.appendChild(d); } }
